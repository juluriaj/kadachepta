...
async function load() {
  ...
  const qcSummary = (item) => {
    if (item.mediaStatus === "pending") return `<p class="qc qc-pending">Checking audio quality…</p>`;
    if (item.mediaStatus === "failed") return `<p class="qc qc-fail">We couldn't process this file. Try exporting it again as MP3 or M4A.</p>`;
    const qc = item.qc || {};
    if (!qc.verdict) return "";
    if (qc.verdict === "pass") return `<p class="qc qc-pass">✓ Audio quality looks good.</p>`;
    return `<ul class="qc qc-${qc.verdict}">${(qc.checks || []).map((check) => `<li><b>${safe(check.message)}</b>${check.tip ? ` ${safe(check.tip)}` : ""}</li>`).join("")}</ul>`;
  };
  published.innerHTML = data.published.length
    ? data.published.map(item => `<article><div><strong>${item.title}</strong><span>${item.album || "Uncategorized"} · v${item.versionNumber || 1} · Published ${item.publishedAt ? item.publishedAt.slice(0, 10) : "pending"}</span><audio controls preload="none" src="${safe(item.audioUrl)}"></audio></div><form class="replacement-form" data-parent="${item.id}"><input name="audio" type="file" accept="audio/*" required><button type="submit">Submit replacement</button></form></article>`).join("")
    : "<p>No published audio yet.</p>";
  document.querySelector("#pipeline").innerHTML = assets.items.length
    ? assets.items.map(item => `<article><div><strong>${item.title}</strong><span>${item.status} · v${item.versionNumber || 1}${item.parentAssetId ? ` · replacement of ${item.parentAssetId}` : ""}</span><audio controls preload="none" src="${safe(item.audioUrl)}"></audio>${qcSummary(item)}</div><span>${item.reviewRequired ? "Review required" : ""}</span></article>`).join("")
